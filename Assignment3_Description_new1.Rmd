...
```
We observe that the highest frequence of the answers to question A takes a middle hypotesis, while for questions B and D the highest frequensy has the answer 2. Most respondents strongly agree with the hypotesis C. The rest of the barplots show the distribution of demographic variables. 
...
Nonlinear MCA is also assumed to be a constrained form of MCA because it is obtained from common MCA (known also as homogenity analysis) by imposing restrictions on the variable ranks and levels as well as defined set of variables. 

The most important advantages of nonlinear over linear MCA are that except trully numeric nonlinear MCA incorporates nominal and ordinal variables, and additionally can discover nonlinear relationships between variables. 
...
Ordinal variables consist of ordered categories, for example as we have also used, a Likert-type scale. Such scale values are not truly numeric because intervals between concequtive categories are not equal. Although ordinal variables display more structure than nominal ones, these variables still do not possess traditional numeric properties. 
...
2. The ordinal quantifications are obtained by placng an order restriction on nominal quantifications.
...
The category points are also positioned on the variable vector, and their coordinates are found by multiplying the category quantifications by the corrtesponding loadings on the first (for the x-coordinate) and the second (for the y-coordinate)component.  
...
![Figure 6](I:\Neli\HYKurssitKevat2017\MultipleCorrespondenceAnalysis\Data+files-20170512\Rplot6A.png)
...
The CA method could be applied on data, which have been preprodessed in different ways. One kind of preprocessing is so called doubling, which in this example is provided on data type ratings. We assume 1-to-5 Linkert scale, which starts at one. Then the first column consists of all ratings minus 1. Since rating 1 = "strongly agree", the transformed first column measure the strength of disagreements and therefore is called *negative pole*. The second column is 4 minus the first column and measure the strength of disagreements. The second column is called *positive pole*. 
This lind of data transformation is called doubling. 
The CA of doubled data is performed as before when we have used nontransformed data.  
Next plot shows the asyymetric map. 
```{r eval=FALSE}
require(ca)
...
```{r}
Finland.fa <- factanal(Finland[,1:4], factors=2, rotation="none", scores="regression")
names(Finland.fa)
print(Finland.fa)
```
...
arrows(0, 0, -Finland.fa$loadings[,1], -Finland.fa$loadings[,2], length=0.1, angle=10, col="pink", lwd=2)
text(-1.05*Finland.fa$loadings, labels=colnames(Finland[,1:4]), col="red", font=4)

Finland.fa.gpc <- matrix(0, nrow=23, ncol=2)
for(k in 1:2) Finland.fa.gpc[1:5,k]  <- tapply(-Finland.fa$scores[,k], INDEX=factor(Finland[,"m"]), mean)
for(k in 1:2) Finland.fa.gpc[6:11,k] <- tapply(-Finland.fa$scores[,k], INDEX=factor(Finland[,"e"]), mean)
for(k in 1:2) Finland.fa.gpc[12:23,k] <- tapply(-Finland.fa$scores[,k], INDEX=factor(Finland[,"ga"]), mean)
rownames(Finland.fa.gpc) <- c("married","widowed","div","sep","single","e0","e1","e2","e3","e4","e5",
                            "ma1","ma2","ma3","ma4","ma5","ma6","fa1","fa2","fa3","fa4","fa5","fa6") 

plot(-1.1*Finland.fa$loadings, type="n", asp=1, xlab="FA factor 1 (26.6%)", ylab="FA factor 2 (7.3%")
abline(h=0, col="gray", lty=2)
abline(v=0, col="gray", lty=2)
arrows(0, 0, -Finland.fa$loadings[,1], -Finland.fa$loadings[,2], length=0.1, angle=10, col="pink", lwd=2)
text(-1.05*Finland.fa$loadings, labels=colnames(Finland[,1:4]), col="red", font=4)
text(0.5*Finland.fa.gpc, labels=rownames(Finland.fa.gpc), col="blue", font=2, cex=0.7)

```

FA with rotation: 
```{r}
Finland.fa <- factanal(Finland[,1:4], factors=2, rotation="varimax", scores="regression")
...
plot(-1.1*Finland.fa$loadings, type="n", asp=1, xlab="FA factor 1 (18.6%)", ylab="FA factor 2 (15.3%")
abline(h=0, col="gray", lty=2)
abline(v=0, col="gray", lty=2)
arrows(0, 0, -Finland.fa$loadings[,1], -Finland.fa$loadings[,2], length=0.1, angle=10, col="pink", lwd=2)
text(-1.05*Finland.fa$loadings, labels=colnames(Finland[,1:4]), col="red", font=4)
...
```{r}
Finland.fa <- factanal(Finland[,1:4], factors=2, rotation="promax", scores="regression")
print(Finland.fa)
```

Plot FA results with oblique rotation:

```{r}
par(mar=c(4.2,4,1,1), mgp=c(2,0.7,0), font.lab=2, cex.axis=0.8)
plot(rbind(-Finland.fa$loadings, -0.5*Finland.fa$scores), asp=1, type="n", xlab="FA factor 1 (16.3%)", ylab="FA factor 2 (16.0%")
abline(h=0, col="gray", lty=2)
abline(v=0, col="gray", lty=2)
points(-0.5*Finland.fa$scores, pch=19, col="lightblue", cex=0.9)
arrows(0, 0, -Finland.fa$loadings[,1], -Finland.fa$loadings[,2], length=0.1, angle=10, col="pink", lwd=2)
...
plot(-1.1*Finland.fa$loadings, type="n", asp=1, xlab="FA factor 1 (16.3%)", ylab="FA factor 2 (16.0%")
abline(h=0, col="gray", lty=2)
abline(v=0, col="gray", lty=2)
arrows(0, 0, -Finland.fa$loadings[,1], -Finland.fa$loadings[,2], length=0.1, angle=10, col="pink", lwd=2)
text(-1.05*Finland.fa$loadings, labels=colnames(Finland[,1:4]), col="red", font=4)
...
Ellipses just by themselves: 
```{r}
require(ellipse)
confidenceplots(-Finland.fa$scores[,1], -Finland.fa$scores[,2], group=Finland$ga, groupcols=c(rep("blue",6),rep("red",6)), shade=T,
                groupnames=c("ma1","ma2","ma3","ma4","ma5","ma6","fa1","fa2","fa3","fa4","fa5","fa6"),shownames=T, add=F)

```

Flip the second axis so that traditional/conservative at the bottom, for easier comparison with MCA: 

```{r}
plot(1.1*cbind(-Finland.fa$loadings[,1],Finland.fa$loadings[,2]), type="n", asp=1, xlab="FA factor 1 (16.3%)", ylab="FA factor 2 (16.0%")
abline(h=0, col="gray", lty=2)
abline(v=0, col="gray", lty=2)
arrows(0, 0, -Finland.fa$loadings[,1], Finland.fa$loadings[,2], length=0.1, angle=10, col="pink", lwd=2)
text(1.05*cbind(-Finland.fa$loadings[,1],Finland.fa$loadings[,2]), labels=colnames(Finland[,1:4]), col="red", font=4)
text(0.5*cbind(Finland.fa.gpc[,1],-Finland.fa.gpc[,2]), labels=rownames(Finland.fa.gpc), col="blue", font=2, cex=0.7)
```
...
Checking p-values in factor analysis for increasing number of dimensions: 

```{r}
for(fac in 2:6) print(factanal(Finland[,1:4], factors=fac, rotation="none", scores="regression")$PVAL)
```

```{r}
for(fac in 2:6) print(factanal(Finland[,1:4], factors=fac, rotation="promax", scores="regression")$PVAL)
```
...
```{r}
Finland.means <- matrix(0,nrow=5,ncol=9)
...
[Package ‘homals’](https://cran.r-project.org/web/packages/homals/homals.pdf)
